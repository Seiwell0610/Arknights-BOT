...
class Admin(commands.Cog):
    ...
    async def webhook_reset(self, ctx):
        if ctx.author.id in admin_list:
            ...
            for channel in global_channels:
                ...
                print(ch_webhooks)
                for cw in ch_webhooks:
                    webhook = discord.utils.get(cw, name=GLOBAL_WEBHOOK_NAME)
    ...
    async def all_guilds(self, ctx):
        if ctx.author.id in admin_list:
            ...
            for count in range(len(guilds)):
                guild = self.bot.get_guild(guilds[count].id)
                pages.append(discord.Embed(title="該当サーバー情報", color=discord.Color.blue()))
